...
public interface PanneRepository extends JpaRepository<Panne, Long>, JpaSpecificationExecutor<Panne> {
    ...
    List<Panne> findByTpeRemplacementId(Long tpeRemplacementId);
    ...
    @Query("SELECT p.typePanne, COUNT(p) FROM Panne p GROUP BY p.typePanne ORDER BY COUNT(p) DESC")
    List<Object[]> countByTypeGrouped();
    
    @Query("SELECT COUNT(p) FROM Panne p WHERE p.statut IN ('REPAREE', 'TESTEE', 'IRRECUPERABLE') AND p.dateResolution BETWEEN :debut AND :fin")
    Long countPannesResoluesDansLaPeriode(LocalDateTime debut, LocalDateTime fin);
    
    @Query("SELECT AVG(TIMESTAMPDIFF(HOUR, p.dateDeclaration, p.dateResolution)) FROM Panne p WHERE p.statut IN ('REPAREE', 'TESTEE', 'IRRECUPERABLE') AND p.dateResolution IS NOT NULL")
    ...
    @Query("SELECT p.typePanne, COUNT(p) as cnt FROM Panne p WHERE p.dateDeclaration BETWEEN :debut AND :fin GROUP BY p.typePanne ORDER BY cnt DESC")
    ...
    @Query("SELECT p.typePanne, COUNT(p) as frequency FROM Panne p GROUP BY p.typePanne ORDER BY frequency DESC")
