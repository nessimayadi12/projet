...
public interface TPEImportRecordRepository extends JpaRepository<TPEImportRecord, Long> {
    ...
    @Query("SELECT r FROM TPEImportRecord r WHERE r.nAffiliation = :nAffiliation ORDER BY r.createdDate DESC, r.id DESC")
    List<TPEImportRecord> findLatestByNAffiliation(@Param("nAffiliation") String nAffiliation);
